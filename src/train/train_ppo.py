...
# 路点观测与距离（提供 obs['waypoint'] 与 info['dist_wp_*']）
from envs.waypoint_wrapper import WaypointWrapper
...
class Trainer:
    ...
    def make_envs(self, curriculum_stage: int) -> list[gymnasium.Env]:
        ...
        for i in range(self.args.experiment.num_procs):
            ...
            env = make_env(self.args.experiment.env, sampler, sequence=True)

            # （先）路点 wrapper：提供 waypoint 与到路点距离
            if getattr(self.args, "waypoint_enable", False):
                env = WaypointWrapper(
                    env,
                    world_scale=getattr(self.args, "waypoint_world_scale", 1.0),
                    mode="centroid",
                )
            ...
            if getattr(self.args, "shaping_enable", False):
                env = PotentialShapingWrapper(
                    ...
                    kappa=getattr(self.args, "shaping_kappa", 0.0),
                )
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument("--shaping.kappa", dest="shaping_kappa",
                        type=float, default=0.0,
                        help="waypoint distance shaping weight (0 to disable)")

    # --- waypoint（点号风格） ---
    parser.add_argument("--waypoint.enable", dest="waypoint_enable",
                        action="store_true", default=False,
                        help="Enable WaypointWrapper to provide obs['waypoint'] and dist_wp_* in info")
    parser.add_argument("--waypoint.world_scale", dest="waypoint_world_scale",
                        type=float, default=1.0,
                        help="Normalize waypoint distances by this world scale")
